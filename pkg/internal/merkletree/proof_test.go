...
func setupTestProofs(t *testing.T) (*MerkleTree, []*mockProof) {
	...
	for i := uint64(0); i < uint64(N); i++ {
		key := []byte(keyPrefix + strconv.FormatUint(i, 10))
	...
	tuple = append(tuple, &mockProof{[]byte(absentKey), nil, absentIndex, ProofOfExclusion})
	m.computeHash()
	return m, tuple
}
...
func TestProofVerificationErrors(t *testing.T) {
	...
	proof2, _ := m.Get(index) // shares the same prefix with leaf node key1
	// assert proof of absence
	if proof2.ProofType() != ProofOfExclusion {
